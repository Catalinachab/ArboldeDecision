...
gen_m_c = function(matriz_conf, filename = NULL) {
  ...
  # Crear un título debajo del heatmap
  title <- textGrob("Matriz de Confusión Normalizada", gp = gpar(fontsize = 14, fontface = "bold"))
  ...
  grid.text("Matriz de Confusión Normalizada", y = unit(0.02, "npc"), gp = gpar(fontsize = 14, fontface = "bold"))
...
TP <- matriz_conf$table[1, 1]  # Verdaderos positivos
TN <- matriz_conf$table[2, 2]  # Verdaderos negativos
FP <- matriz_conf$table[1, 2]  # Falsos positivos
FN <- matriz_conf$table[2, 1]  # Falsos negativos

# Calcular las proporciones
total <- sum(matriz_conf$table)

TP_prop <- TP / total * 100  # Proporción de TP
TN_prop <- TN / total * 100  # Proporción de TN
FP_prop <- FP / total * 100  # Proporción de FP
FN_prop <- FN / total * 100  # Proporción de FN

# Crear texto con las métricas enumeradas
texto_resultado <- paste(
  "En la matriz de confusión podemos observar:\n",
  "1. *TP* (Verdaderos Positivos): ", round(TP_prop, 2), "% - Proporción de verdaderos positivos en relación con el total de verdaderos positivos.\n",
  "2. *TN* (Verdaderos Negativos): ", round(TN_prop, 2), "% - Proporción de verdaderos negativos en relación con el total de verdaderos negativos.\n",
  "3. *FP* (Falsos Positivos): ", round(FP_prop, 2), "% - Proporción de falsos positivos en relación con el total de verdaderos negativos.\n",
  "4. *FN* (Falsos Negativos): ", round(FN_prop, 2), "% - Proporción de falsos negativos en relación con el total de verdaderos positivos.\n",
  sep = ""
)
