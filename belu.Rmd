...
Los mejores hiperparámetros encontrados en la optimización son:

```{r, echo=FALSE, warning=FALSE, message=FALSE}
cat("max_depth =", res$maxdepth, 
     ", min_split =", res$minsplit, 
     ", min_bucket =", res$minbucket,"\n")
...
# print(res)
# 
# cat("Hiperparámetros: max_depth =", tree$control$maxdepth, 
#     ", min_split =", tree$control$minsplit, 
#     ", min_bucket =", tree$control$minbucket, "\n")
...
metricas_comparativa2 <- data.frame(
  ...
  Modelo_Default = c(accuracy_modelo1, precision_modelo1, recall_modelo1, f1_modelo1 ),
  Modelo_Opt = c(accuracy_modelo2, precision_modelo2, recall_modelo2, f1_modelo2),
...
variables_d <- variables_d[order(variables_d$imp), ]

# Crear el gráfico
ggplot(variables_d, aes(x = reorder(variable, imp), y = imp)) +
  geom_bar(stat = "identity", fill = "#E07A5F", ) +
  coord_flip() +
  theme_minimal() +
  labs(title = "Variable Importance del modelo default", x = NULL, y = "Variable Importance reescalada")


#VI con modelo optimizado 
...
# Ordenar por importancia
variables <- variables_o[order(variables_o$imp), ]

# Crear el gráfico
ggplot(variables_o , aes(x = reorder(variable, imp), y = imp)) +
  geom_bar(stat = "identity", fill = "#6AA4C1", ) +
  coord_flip() +
  theme_minimal() +
  labs(title = "Variable Importance del modelo optimizado", x = NULL, y = "Variable Importance reescalada")

```

En estos gráficos observamos que, tanto en el modelo original con los hiperparámetros por defecto como en el modelo optimizado, la importancia de las variables se mantiene igual. Destaca especialmente previous_loan_defaults_on_file, lo que indica que el historial de incumplimiento de préstamos anteriores es un factor clave en las predicciones. Además, la relación entre el monto del préstamo y los ingresos del solicitante, así como la tasa de interés, tienen un impacto significativo. Otros factores relevantes incluyen los ingresos personales, la propiedad de vivienda y el puntaje de crédito. Por otro lado, algunas variables, como el género y la educación, parecen tener poca influencia y podrían ser eliminadas para simplificar el modelo sin perder su precisión.
...
# Imprimir la tabla
kable(metricas_comparativa_NA, caption = "Comparativa de métricas entre los modelos con valores faltantes", format = "latex", booktabs = TRUE) 
...
ggplot(roc_data, aes(x = 1 - specificity, y = sensitivity, color = modelo)) +
  ...
  labs(
    ...
    y = "True Positive Rate (Sensitivity)",
    title = "Comparación de Curvas ROC para Diferentes Modelos",
    color = "Modelo"
  ) +
...
ggplot(df_long, aes(x = Métrica, y = Proporción, fill = Modelo)) +
  ...
  labs(title = "Desempeño Relativo respecto al Modelo Optimizado",
       x = "Métrica",
